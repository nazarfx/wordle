...
io.on('connection', (socket) => {
    ...
    // НОВОЕ СОБЫТИЕ: Перезапуск внутри той же комнаты для всех
    socket.on('restartGame', ({ roomId, nextWord }) => {
        const room = rooms[roomId];
        if (!room) return;

        // 1. Присваиваем новое секретное слово текущей комнате
        let secretWord = nextWord ? nextWord : "столб";
        room.secretWord = secretWord.toLowerCase().replace(/ё/g, 'е');

        // 2. Обнуляем прогресс каждого игрока в комнате
        for (const id in room.players) {
            room.players[id].progress = [];
            room.players[id].currentAttempt = 0;
            room.players[id].score = 'playing';
        }

        // 3. Отправляем сигнал рестарта и обновленные пустые доски всем в комнате
        io.to(roomId).emit('gameRestarted');
        io.to(roomId).emit('roomUpdated', { roomId, players: room.players });
        console.log(`[КОМНАТА ${roomId}] Игра перезапущена. Новое слово: ${room.secretWord}`);
    });
