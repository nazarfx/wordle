...
// Сервер сообщает, что вся комната перезапущена
...
function checkRow() {
    ...
    // Для валидации используем полный словарь
...
// Кнопка рестарта ТЕПЕРЬ НЕ СОЗДАЕТ НОВУЮ комнату, а просит сервер обнулить ТЕКУЩУЮ
restartBtn.addEventListener("click", () => {
    if (currentRoomId) {
        // Выбираем новое простое слово прямо на фронтенде из WORDLE_SECRET_WORDS
        const nextWord = WORDLE_SECRET_WORDS[Math.floor(Math.random() * WORDLE_SECRET_WORDS.length)];
        socket.emit('restartGame', { roomId: currentRoomId, nextWord });
    }
});
...
// });
//
// if (typeof WORDLE_DICTIONARY !== 'undefined' && WORDLE_DICTIONARY.length > 0) {
//     const firstWord = WORDLE_DICTIONARY[Math.floor(Math.random() * WORDLE_DICTIONARY.length)];
//     socket.emit('createRoom', { secretWord: firstWord });
// } else {
//     socket.emit('createRoom');
// }
